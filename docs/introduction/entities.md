...
Data factory have to prepare this array to contain keys for all domain IDs, because domain entities are created by these array items (even if their value is null).

Multilanguage field like `description` have to indexed by `locale` - string identifier of language (you can find them in [`domains.yml`](/project-base/app/config/domains.yml)).
Data factory do not prepare this arrays, because the extension we use for translated entities can handle unprepared translations.
